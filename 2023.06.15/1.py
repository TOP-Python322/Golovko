...
def pick_resistors(resistance: int) -> dict | None:
    """Pick the suitable value of resistor
    from several precision lists"""
    ...
    for item in nominals.keys():
        _ = nominals[item]
        for i in _:
            t = tuple(filter(lambda x: resistance >= x * (1 - faults[item] / 100) and resistance <= x * (1 + faults[item] / 100), _))
            if t == ():
                min_diff = min(list(map(lambda x : abs(x - resistance), _)))
                t = tuple(filter(lambda x : abs(x - resistance) == min_diff, _))
